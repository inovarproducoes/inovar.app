                ...
                {[
                  ...
                  { label: "Projeto", key: "projeto", icon: FileText }
                    ...
                    <div key={field.key} className="space-y-1.5">
                        ...
                        {isEditMode ? (
                            <Input value={(alunoSelecionado as unknown as Record<string, string | null | undefined>)?.[field.key] || ""} onChange={e => setAlunoSelecionado(a => a ? {...a, [field.key]: e.target.value} : null)} className="h-9 bg-muted/30 border-primary/20 text-xs" />
                        ) : (
                            <p className="text-sm font-dm pl-2 border-l-2 border-primary/10">{(alunoSelecionado as unknown as Record<string, string | null | undefined>)?.[field.key] || '--'}</p>
                        )}
                    </div>
                ))}
            </div>
