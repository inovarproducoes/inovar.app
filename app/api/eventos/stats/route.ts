...
export async function GET() {
  try {
    ...
    const [todosEventos, proximosEventos, recentes] = await Promise.all([
    ...
    todosEventos.forEach((e) => {
      ...
      if (!isNaN(data.getTime())) eventosPorMesMap[data.getMonth()]++;
      if (e.tipo_evento) {
        distribuicaoMap[e.tipo_evento] = (distribuicaoMap[e.tipo_evento] || 0) + 1;
      }
    });

    const eventosPorMes = eventosPorMesMap.map((total, idx) => ({ name: monthNames[idx], total }));
    ...
    return NextResponse.json(
      {
        ...
        vagasOcupadas,
        taxaOcupacao,
        tendencias: { total: 12, vagas: 8, taxa: 5 },
