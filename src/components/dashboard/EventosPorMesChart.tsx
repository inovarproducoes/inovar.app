...
interface EventosPorMesChartProps {
  data: { name: string; total: number }[];
}

export function EventosPorMesChart({ data }: EventosPorMesChartProps) {
  return (
    <Card className="col-span-1 lg:col-span-2 shadow-sm border-white/5 bg-card/60 backdrop-blur-xl">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <div className="flex flex-col space-y-1">
          <CardTitle className="text-base font-bold">Eventos por Mês</CardTitle>
          <CardDescription className="text-xs">Distribuição ao longo do ano</CardDescription>
        </div>
